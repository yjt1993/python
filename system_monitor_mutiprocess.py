...
NETT = 2  # 计算网卡流量的时间间隔
FILE = '/home/hduser/result/192.168.4.232'
    ...
    while i < 20:
        try:
            process_info = psutil.Process(
                int(os.popen("ps aux|grep -v PID|sort -rn -k +3").readlines()[i].split()[1]))  # 获取进程信息
            pid_bak = process_info.pid  # 获取pid
            process_status_bak = process_info.status()  # 获取进程状态
            process_user_bak = process_info.username()  # 获取进程用户
            process_name_bak = ' '.join(
                os.popen('ps aux |grep -v PID|sort -k3 -nr').readlines()[i].split()[10:])  # 获取进程名字
            process_cpu_percent_bak = ''.join(
                os.popen('ps aux |grep -v PID|sort -k3 -nr').readlines()[i].split()[2])  # 获取进程CPU使用
...
# 获取磁盘信息
def diskinfo():
    if os.path.exists("/dev/vda"):
        disk_num = int(''.join(os.popen("ls /dev/vd[a-z]|wc -l").readlines()[0].split()))
    else:
...
# 获取hadoop job list
def jobinfo():
    hadoop_process = ['JournalNode', 'ResourceManager', 'HMaster', 'DataNode', 'DFSZKFailoverController',
                      'QuorumPeerMain', 'HQuorumPeerMain', 'JobHistory', 'Kafka', 'NodeManager', 'Worker', 'Master',
                      'HRegionServer', 'NameNode', 'PrestoServer', 'RunJar', 'NiFi']
    job_value = []
    # 获取java环境变量
    JAVA_HOME = os.environ['JAVA_HOME']
    job_lst = os.popen(os.path.join(JAVA_HOME, 'bin/jps')).readlines()
    for i in job_lst:
        value = i.split()[1]
...
if __name__ == '__main__':
    ...
    func_list = ['baseinfo','cpuinfo','meminfo','diskinfo','netinfo','tcpinfo','jobinfo']
    pool = Pool()
    result_data = {}
    middle_result  = []
    for i in func_list:
        res = pool.apply_async(eval(i))
        middle_result.append(res)
    pool.close()
    pool.join()
    for i in middle_result:
        result_data.update(i.get())
